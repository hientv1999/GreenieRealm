...
$current_password = $new_email = $confirm_email = "";
$current_password_err = $new_email_err = $confirm_email_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($current_password_err) &&  empty($new_email_err) && empty($confirm_email_err)){
        $result = $db_handle->runBaseQuery("SELECT email, password FROM users WHERE username = '$current_user' LIMIT 1");
        //Validate current password
        if(!password_verify($current_password, $result[0]["password"])){
            ...
            if ($new_email != $result[0]["email"]){
                ...
                $db_handle->runBaseQuery("UPDATE users SET email = '$new_email' WHERE id = '$param_id'");
                $_SESSION["email"] = $new_email;
                header("location: setting.php");
                exit();
            } else {
                $new_email_err = "Please use different email";
            }
...
<meta charset="utf-8" />
        ...
        <title><?php echo htmlspecialchars($_SESSION["username"]); ?> - Change Email</title> 
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-5">
                            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post"> 
                                ...
                                <p style="text-align: center">Please fill out this form to change your email.</p>
                                ...
                                <div class="form-group <?php echo (!empty($new_email_err)) ? 'has-error' : ''; ?>">
                                    ...
                                    <span style="font-size: 14px; color: #fc8f8f;"><?php echo $new_email_err; ?></span>
                                </div>
                                <div class="form-group <?php echo (!empty($confirm_email_err)) ? 'has-error' : ''; ?>">
                                    ...
                                    <span style="font-size: 14px; color: #fc8f8f;"><?php echo $confirm_email_err; ?></span>
                                </div>
